...
test.describe( `${ blockData.name }`, () => {
	...
	test.describe.only( 'full-screen when clicked option', () => {
		...
		test.only( 'should open dialog on the frontend', async ( {
			pageObject,
			page,
			editorUtils,
		} ) => {
			...
			await expect( mainImageBlock.locator( 'dialog' ) ).toBeHidden();
			await mainImageBlock.click();

			await expect( mainImageBlock.locator( 'dialog' ) ).toBeVisible();
		} );
		...
		test( 'should not open dialog when the setting is disable on the frontend', async ( {
			...
			const mainBlock = await pageObject.getMainImageBlock( {
				page: 'frontend',
			} );

			await expect( mainBlock.locator( 'dialog' ) ).toBeHidden();
